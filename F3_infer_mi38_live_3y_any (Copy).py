...
- body_strong đa khung: tính |close - open|/open * 100 với ngưỡng theo hướng LONG/SHORT
  * BODY_STRONG_TFS = ["15m", "30m", "1h"] (chọn một hay nhiều khung)
  * BODY_STRONG_REQUIRE_MODE = "ANY" (chỉ cần 1 khung đạt) hoặc "ALL" (mọi khung phải đạt)
"""
...
BODY_STRONG_PCT      = 0.5           # 0.3% theo yêu cầu
# ➜ KHUNG THỜI GIAN ÁP BODY-STRONG (bạn chỉnh tùy ý: "15m", "30m", "1h")
BODY_STRONG_TFS      = ["15m"]       # ví dụ: ["15m","30m"] hoặc ["15m","30m","1h"]
# ➜ Chế độ yêu cầu: "ANY" (chỉ cần 1 khung đạt) hoặc "ALL" (mọi khung phải đạt)
...
# ============= DEVICE SAFE (Auto: NVIDIA CUDA hoặc Intel XPU/ITEX) =============
def init_device():
    import tensorflow as tf

    def _enable_tf32_safe():
        # TF32 chủ yếu hữu ích khi chạy trên NVIDIA; để try/except cho an toàn.
        try:
            tf.config.experimental.enable_tensor_float_32_execution(True)
        except Exception:
            pass
    ...
    try:
        # 1) Ưu tiên phát hiện NVIDIA GPU (device type "GPU")
        gpus = tf.config.list_physical_devices("GPU")
        if gpus:
            print("✅ NVIDIA GPU detected:", gpus)
            # Bật memory growth để tránh chiếm hết VRAM ngay từ đầu
            for g in gpus:
                try:
        ...
        # 2) Nếu không có NVIDIA, kiểm tra Intel XPU (PluggableDevice của ITEX)
        xpus = tf.config.list_physical_devices("XPU")
        if xpus:
            print("✅ Intel XPU detected (ITEX):", xpus)
            try:
                import intel_extension_for_tensorflow as itex
                # 'AUTO' sẽ chọn Intel GPU nếu có, fallback CPU nếu không
                itex.set_backend("AUTO")
            except Exception as e:
                print("⚠️  ITEX import/init failed:", e)
            # In ra logical XPU để xác nhận
            try:
                print("Logical XPU devices:",
                      [d.name for d in tf.config.list_logical_devices("XPU")])
            except Exception:
                pass
            return "INTEL_XPU"

        # 3) Không tìm thấy GPU/XPU → chạy CPU
        print("⚠️  No NVIDIA/Intel GPU detected. Running on CPU.")
        return "CPU"
...
def pass_filters_for(direction: str, ema_info: Dict[str, float], body_tfs: List[str]) -> Tuple[bool, str]:
    ...
    if REQUIRE_BODY_STRONG:
        body_map = get_body_pcts(symbol, body_tfs)  # <- cần symbol; ta sẽ truyền bằng closure trong main
...
def main():
    ...
    # Ta bọc pass_filters_for thành hàm local để chuyền symbol vào gọn hơn
    def run_filters(direction: str, sym: str) -> Tuple[bool, str]:
        ema_info = get_ema_flags(sym)  # theo INTERVAL giao dịch
        # body_strong theo BODY_STRONG_TFS
        # pass_filters_for sẽ gọi get_body_pcts thông qua biến 'symbol' ở scope ngoài
        # => ta truyền 'symbol' bằng biến toàn cục tạm (Python closure trick)
        global symbol
        symbol = sym  # tạm đặt để pass_filters_for truy cập
        try:
