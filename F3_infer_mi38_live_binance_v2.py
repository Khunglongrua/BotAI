    ...
    if webhook and signal in {"LONG","SHORT"}:
        ...
        emb = {
            ...
            "fields": [
                ...
                {"name": "price", "value": (f"{price:.6f}" if (price is not None and np.isfinite(price)) else "-"), "inline": True},
                {"name": "thr | reason", "value": f"{THR:.2f}% | {reason}", "inline": True},
            ],
            "footer": {"text": f"F3_mi38 live • ws={WS} • Hôm nay lúc {time.strftime('%H:%M', time.localtime())}"},
